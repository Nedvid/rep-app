...
function createServerForCurrentEnvironment() {
  if (process.env.ENVIRONMENT === 'dev') {
    return createHttpsServer({
        key: fs.readFileSync(path.resolve(__dirname, '../../../src/local_ssl/localhost.key')),
        cert: fs.readFileSync(path.resolve(__dirname, '../../../src/local_ssl/localhost.crt'))
    }, requestListener);
  }
...
const server = createServerForCurrentEnvironment();
server.listen(PORT, () => {
